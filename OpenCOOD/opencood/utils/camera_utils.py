...
def load_rgb_from_files(camera_list):
    ...
    with ThreadPoolExecutor(max_workers=min(8, len(camera_list))) as executor:
        futures = [executor.submit(_read_image, (i, f)) for i, f in enumerate(camera_list)]
        for future in as_completed(futures):
            i, img = future.result()
            camera_dict[f'camera{i}'] = img
    camera_dict = OrderedDict(sorted(camera_dict.items(), key=lambda x: int(x[0].replace('camera', ''))))
    return camera_dict

# def load_rgb_from_hdf5_file(hdf5_path, expected_num=5):
#     camera_dict = OrderedDict()
#     with h5py.File(hdf5_path, 'r') as f:
#         cam_ids = [int(k.replace("camera", "")) for k in f.keys()]
#         min_id = min(cam_ids)
#         max_id = max(cam_ids)
#         sorted_ids = sorted(cam_ids)

#         for idx in range(expected_num):
#             real_id = min_id + idx
#             key = f"camera{real_id}"
#             if key in f:
#                 img = f[key][()]
#             else:
#                 # 假设已知图像大小为 256x256x3
#                 img = np.zeros((1080, 1920, 3), dtype=np.uint8)
#             camera_dict[f'camera{idx}'] = img
#     return camera_dict
...
#         images = list(pool.map(_read_image, camera_list))
#     return {f'camera{i}': img for i, img in enumerate(images)}
