...
def remove_bbx_abnormal_z(bbx_3d, min_size=0.1):
    """
    Remove bounding boxes that have abnormal z or invalid size (dx or dy <= 0)

    Parameters
    ----------
    bbx_3d : torch.Tensor
        Predicted 3d bounding boxes, shape: (N, 8, 3)
    min_size : float
        Minimum acceptable size for dx, dy
    ...
    bbx_z_min = torch.min(bbx_3d[:, :, 2], dim=1)[0]
    bbx_z_max = torch.max(bbx_3d[:, :, 2], dim=1)[0]
    z_index = torch.logical_and(bbx_z_min >= -3, bbx_z_max <= 1)

    # dx dy 尺寸筛选
    dx = torch.norm(bbx_3d[:, 0, :2] - bbx_3d[:, 1, :2], dim=1)
    dy = torch.norm(bbx_3d[:, 1, :2] - bbx_3d[:, 2, :2], dim=1)
    size_index = torch.logical_and(dx > min_size, dy > min_size)

    # 组合筛选条件
    final_index = torch.logical_and(z_index, size_index)
...
def project_points_by_matrix_torch(points, transformation_matrix):
    ...
    # projected_points = points_homogeneous @ transformation_matrix.T
